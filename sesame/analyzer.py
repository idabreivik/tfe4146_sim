...
class Analyzer():
    ...
    def get_band_edge(self, location, band='c'):
        """ Return values for the conduction/valence band edge

        Args:
            location: array-like ((x1,y1), (x2,y2))
                Tuple of two points defining a line over which to compute a band
                diagram.
            band (str, optional): 'v' or 'c'. Defaults to 'c'.
        """
        p1, p2 = location
        ...
        idx2, _ = get_indices(self.sys, (p2[0],0,0))
        X = self.sys.xpts[idx1:idx2]
        sites = np.arange(idx1, idx2+1, 1, dtype=int)

        vt = self.sys.scaling.energy
        X = X * 1e4  # in um

        if band == 'v':
            return -vt * (self.v[sites] + self.sys.bl[sites] + self.sys.Eg[sites])
        elif band == 'c':
            return -vt * (self.v[sites] + self.sys.bl[sites])
        else:
            raise ValueError(f'Invalid band type {band}')

    def get_electrostatic_potential(self, location):
        """ Return the electrostatic potential across the junction

        Args:
            location: array-like ((x1,y1), (x2,y2))
                Tuple of two points defining a line over which to compute the electrostatic potential.
        """
        # Band energy in eV
        ec = self.get_band_edge(location)
        # Potential difference in volt. Let zero volt be the lowest value
        v = -ec
        v -= np.min(v)
        return v


    def get_e_field(self, location):
        """ Return the electric field across the junction
        ...
        v = self.get_electrostatic_potential(location)
    ...
    def get_space_charge_density(self, location):
        """ Return the electric field across the junction
        ...
        E = self.get_e_field(location)
        epsilon = self.sys.epsilon * 8.85e-14# cm^-3
        p1, p2 = location
        dx = (p2[0] - p1[0])/len(E)
        return epsilon* np.gradient(E, dx)
